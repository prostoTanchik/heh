...
public class NewsPage extends SuperPage {
    ...
    public NewsPage() {
        ...
        AjaxButton newPost = new AjaxButton("newPost") {
            ...
            protected void onSubmit(AjaxRequestTarget target) {
                usersService.savePost(currentUserModel.getObject(), postModel.getObject());
                postsModel.setObject(postRepository.findFriendsPostsByFriends(currentUserModel.getObject()));
        ...
        postsModel = new ListModel<>(postRepository.findFriendsPostsByFriends(currentUserModel.getObject()));
            ...
            protected void populateItem(ListItem<Post> item) {
                DynamicImageResource resource = new DynamicImageResource(){
                    ...
                    protected byte[] getImageData(Attributes attributes) {
                        return item.getModelObject().getUser().getAccount().getAvatar();
                    }
                };

                Image avatar;
                if (item.getModelObject().getUser().getAccount().getAvatar() == null) {
                ...
                item.add(new Label("author", "Автор: " + item.getModelObject().getUser().getAccount().getFullName()));
                Form<Void> form = new Form("form");
                ...
                AjaxButton delete = new AjaxButton("delete") {
                    @Override
                    protected void onSubmit(AjaxRequestTarget target) {
                        usersService.deletePost(currentUserModel.getObject(), item.getModelObject());
